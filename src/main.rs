...
use ggez::graphics::{self, DrawMode, Drawable, DrawParam, Canvas};
use ggez::Context;
...
impl ggez::event::EventHandler for MainState {
    ...
    fn key_down_event(
        ...
        match keycode {
            ...
            KeyCode::E => self.mand.zoom *= 1.0 - 1.0/10.00000001,
            KeyCode::Q => {
                println!("MainState\n=========\n {:#?}", &self);
                ggez::quit(ctx);
            }
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        graphics::clear(ctx, [0.1, 0.1, 0.3, 1.0].into());
        {
            ...
            self.shad.send(ctx, self.mand).unwrap();
